...
def summary(db: Session) -> dict:
    total_planned = db.scalar(select(func.sum(WorkOrderItem.planned_carbon_kgco2e))) or 0.0
    total_actual = db.scalar(select(func.sum(_EFFECTIVE)).select_from(ExecutionRecord)) or 0.0

    # Per-hectare from actual surface across executions (normalize units in Python).
    rows = db.execute(
        select(ExecutionRecord.actual_surface_area_value, ExecutionRecord.actual_surface_area_unit,
               _EFFECTIVE)
    ).all()
    ...
    overrides = db.scalar(
        select(func.count(ExecutionRecord.id)).where(ExecutionRecord.carbon_override_value.is_not(None))
    ) or 0
    ...
    return {
        ...
        "top_activities": by_activity(db)[:5],
        "top_products": by_product(db)[:5],
    }


def _grouped(db: Session, label_col, *, join_wo: bool = False) -> list[tuple]:
...
def by_activity(db: Session) -> list[dict]:
    rows = _grouped(db, ExecutionRecord.activity_id)
    out = []
...
def by_product(db: Session) -> list[dict]:
    rows = _grouped(db, ExecutionRecord.product_id)
    out = []
...
def by_lot(db: Session) -> list[dict]:
    rows = _grouped(db, WorkOrder.lot_id, join_wo=True)
    out = []
...
def by_field(db: Session) -> list[dict]:
    rows = _grouped(db, WorkOrder.field_id, join_wo=True)
    out = []
...
def by_season(db: Session) -> list[dict]:
    rows = _grouped(db, WorkOrder.season_id, join_wo=True)
    return [{"season_id": s, "kgco2e": _round(t)} for s, t in rows]


def missing_data_report(db: Session, limit: int = 200) -> list[dict]:
    rows = db.execute(
...
def override_report(db: Session, limit: int = 200) -> list[dict]:
    rows = db.execute(
        select(ExecutionRecord).where(ExecutionRecord.carbon_override_value.is_not(None)).limit(limit)
    ).scalars().all()
